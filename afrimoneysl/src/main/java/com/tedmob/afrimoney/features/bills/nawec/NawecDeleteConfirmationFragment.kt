...
@AndroidEntryPoint
class NawecDeleteConfirmationFragment :
    BaseVBFragment<FragmentNawecDeleteConfirmationBinding>() {
    ...
    override fun onCreateView(
        ...
        return createViewBinding(container, FragmentNawecDeleteConfirmationBinding::inflate)
    }
    ...
    private fun FragmentNawecDeleteConfirmationBinding.observeData() {
        withVBAvailable {

            setupData()

            observeTransactionSubmit(viewModel.confirmDeleteData, binding!!.confirmButton) {
                exit()
            }
    ...
    private fun FragmentNawecDeleteConfirmationBinding.setupData() {
        binding?.meterNumber?.text = viewModel.meterId
        binding?.nicknameText?.text = viewModel.nickname

    }

    private inline fun FragmentNawecDeleteConfirmationBinding.setupValidator() = formValidator {
        pinInputLayout.validate(NotEmptyRule(getString(R.string.mandatory_field)))

        onValid = {

            viewModel.getDeleteConfirmation(pinInputLayout.getText())
        }
    }
