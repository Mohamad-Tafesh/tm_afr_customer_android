...
class DatePickerSpinnerFragment : DialogFragment() {
    ...
    fun setDateInMillis(millis: Long) {
        setDate(Calendar.getInstance().apply { timeInMillis = millis })
    }

    fun setDate(calendar: Calendar) {
        setDate(calendar[Calendar.YEAR], calendar[Calendar.MONTH], calendar[Calendar.DAY_OF_MONTH])
    }

    /**
     * @param year: Year value like the [Calendar] field: [Calendar.YEAR]
     * @param month: Month value like the [Calendar] field: [Calendar.MONTH] (zero-based)
     * @param day: Day value like the [Calendar] field: [Calendar.DAY_OF_MONTH]
     */
    fun setDate(year: Int, month: Int, day: Int) {
        ...
        datePicker?.setDate(year, month + 1, day)//in case the UI was available
    }
            ...
            minDate?.let {
                if (currentCalendar[Calendar.YEAR] == it[Calendar.YEAR]) {
                    ...
                    monthPicker.displayedValues =
                        displayedMonthValues.copyOfRange(it[Calendar.MONTH], displayedMonthValues.size)
