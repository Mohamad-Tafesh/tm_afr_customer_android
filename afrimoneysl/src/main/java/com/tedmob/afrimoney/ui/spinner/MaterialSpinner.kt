...
import com.tedmob.afrimoney.ui.spinner.searchable.MaterialSearchableListDialogHolder
...
open class MaterialSpinner @JvmOverloads constructor(
    ...
    //image:
    protected fun setImageFromProvider(provider: ImageProvider) {
        if (Looper.myLooper() == Looper.getMainLooper()) {
            setImage(provider.getImage(context))
        } else {
            post {
                setImage(provider.getImage(context))
            }
    ...
    @MainThread
    private inline fun setImage(drawable: Drawable?) {
        editText.doOnLayout {
            when (drawable) {
                is BitmapDrawable? -> {
                    if (drawable != null) {
                        //attempt to match the height of the editText while keeping the same aspect ratio:
                        val originalWidth = drawable.intrinsicWidth.takeIf { it >= 0 }
                        val originalHeight = drawable.intrinsicHeight.takeIf { it >= 0 }

                        val scaledHeight = editText.measuredHeight - editText.paddingTop - editText.paddingBottom
                        val scaledWidth =
                            if (originalWidth != null && originalHeight != null)
                                ((originalWidth.toDouble() / originalHeight) * scaledHeight).roundToInt()
                            else
                                originalWidth ?: 0

                        val scaledDrawable = drawable.bitmap
    ...
    private val COLLAPSING_HELPER = "collapsingTextHelper"
    private val COLLAPSED_BOUNDS = "collapsedBounds"
    private val RECALCULATE = "recalculate"

    private val collapsingTextHelper: Any? by lazy {
        try {
    ...
    private val bounds: Rect? by lazy {
        try {
            ...
            boundsField?.get(collapsingTextHelper) as Rect
        } catch (e: NoSuchFieldException) {
            e.printStackTrace()
            null
        } catch (e: IllegalAccessException) {
    ...
    private val recalculateMethod: Method? by lazy {
        try {
            collapsingTextHelper?.javaClass?.getDeclaredMethod(RECALCULATE)
        } catch (e: NoSuchFieldException) {
            e.printStackTrace()
            null
        } catch (e: IllegalAccessException) {
    ...
    private inline fun containsImage(): Boolean =
        editText.compoundDrawablesRelative.getOrNull(0) != null

    private inline fun adjustHintBounds() {
