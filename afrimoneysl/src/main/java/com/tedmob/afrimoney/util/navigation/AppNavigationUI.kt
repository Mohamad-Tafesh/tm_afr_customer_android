...
fun NavController.onNavDestinationIdSelected(@IdRes idRes: Int): Boolean {
    val builder = NavOptions.Builder().setLaunchSingleTop(true).setRestoreState(true)
    if (
        currentDestination?.parent?.findNode(idRes) is ActivityNavigator.Destination
    ) {
        builder
    ...
    return try {
        ...
        navigate(idRes, null, options)
        // Return true only if the destination we've navigated to matches the id
        currentDestination?.hierarchy?.any { it.id == idRes } == true
