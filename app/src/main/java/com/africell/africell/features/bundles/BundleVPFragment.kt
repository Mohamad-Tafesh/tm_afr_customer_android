...
import com.africell.africell.databinding.FragmentBundleVpBinding
import com.africell.africell.databinding.ToolbarBundleVpBinding
...
class BundleVPFragment : BaseVBFragment<FragmentBundleVpBinding>() {
    ...
    private fun searchRxTextView() {
        getToolbarBindingAs<ToolbarBundleVpBinding>()?.run {
            searchTxt?.let {
    ...
    private fun bindData() {
        ...
        observeResourceInline(viewModel.bundlesData) { bundles ->
            ...
            val firstSecondaryPrimaryKey =
                bundles.getOrNull(0)?.bundleInfo?.getOrNull(0)?.secondaryColor ?: secondaryColor
            changeBgdColor(firstPrimaryKey, firstSecondaryPrimaryKey)
            if (bundleName.isNullOrEmpty()) {
                actionbar?.title =
                    bundles.getOrNull(0)?.bundleInfo?.getOrNull(0)?.category.orEmpty() + getString(R.string._bundles)
            }
    ...
    private fun setupViewPager(bundles: List<BundlesDTO>) {
        withVBAvailable {
            ...
            viewPager.adapter = object : FragmentStateAdapter(this@BundleVPFragment) {
