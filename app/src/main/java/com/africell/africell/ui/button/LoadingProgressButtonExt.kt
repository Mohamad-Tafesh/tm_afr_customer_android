...
open class LiveDataObserver<T> {
    ...
    open var overrideDefaultOnError: ((message: String, action: (() -> Unit)?) -> Unit)? = null
    ...
    open var onErrorWhenLoading: (message: String, action: (() -> Unit)?) -> Unit = { _, _ -> }
    open var onError: (message: String, action: (() -> Unit)?) -> Unit = { _, _ -> }
